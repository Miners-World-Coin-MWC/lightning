...
const char *channeld_wire_name(int e)
{
	...
	case WIRE_CHANNELD_PING: return "WIRE_CHANNELD_PING";
	case WIRE_CHANNELD_PING_REPLY: return "WIRE_CHANNELD_PING_REPLY";
	}
...
u8 *towire_channeld_ping(const tal_t *ctx, u16 num_pong_bytes, u16 len)
{
	u8 *p = tal_arr(ctx, u8, 0);

	towire_u16(&p, WIRE_CHANNELD_PING);
	towire_u16(&p, num_pong_bytes);
	towire_u16(&p, len);

	return memcheck(p, tal_count(p));
}
bool fromwire_channeld_ping(const void *p, u16 *num_pong_bytes, u16 *len)
{
	const u8 *cursor = p;
	size_t plen = tal_count(p);

	if (fromwire_u16(&cursor, &plen) != WIRE_CHANNELD_PING)
		return false;
...
/* WIRE: CHANNELD_PING_REPLY */
u8 *towire_channeld_ping_reply(const tal_t *ctx, bool sent, u16 totlen)
{
	u8 *p = tal_arr(ctx, u8, 0);

	towire_u16(&p, WIRE_CHANNELD_PING_REPLY);
...
bool fromwire_channeld_ping_reply(const void *p, bool *sent, u16 *totlen)
{
	const u8 *cursor = p;
	size_t plen = tal_count(p);

	if (fromwire_u16(&cursor, &plen) != WIRE_CHANNELD_PING_REPLY)
		return false;
 	/* False if we there was already a ping in progress. */
...
// SHA256STAMP:08a383cdfaae3869c3e83a6a9287f38a73ed17701cfd9ba7f7b93654a3ad5b55
