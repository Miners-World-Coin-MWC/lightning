...
void towire_blinded_path(u8 **p, const struct blinded_path *blinded_path)
{
	...
	towire_pubkey(p, &blinded_path->first_node_id);
...
fromwire_blinded_path(const tal_t *ctx, const u8 **cursor, size_t *plen)
{
	...
	u8 num_hops;
 	...
 	fromwire_pubkey(cursor, plen, &blinded_path->first_node_id);
...
/* OFFER MSG: issuer */
static u8 *towire_tlv_offer_issuer(const tal_t *ctx, const void *vrecord)
{
	const struct tlv_offer *r = vrecord;
	u8 *ptr;
	...
	towire_utf8_array(&ptr, r->issuer, tal_count(r->issuer));
	return ptr;
}
static void fromwire_tlv_offer_issuer(const u8 **cursor, size_t *plen, void *vrecord)
{
	struct tlv_offer *r = vrecord;

	r->issuer = tal_arr(r, utf8, *plen);
fromwire_utf8_array(cursor, plen, r->issuer, *plen);
}
...
static void fromwire_tlv_offer_node_id(const u8 **cursor, size_t *plen, void *vrecord)
{
	struct tlv_offer *r = vrecord;
	    ...
	    r->node_id = tal(r, struct point32);

fromwire_point32(cursor, plen, &*r->node_id);
}
...
static const struct tlv_record_type tlvs_offer[] = {
	...
	{ 20, towire_tlv_offer_issuer, fromwire_tlv_offer_issuer },
...
/* INVOICE_REQUEST MSG: chain */
static u8 *towire_tlv_invoice_request_chain(const tal_t *ctx, const void *vrecord)
{
	const struct tlv_invoice_request *r = vrecord;
	u8 *ptr;
	...
	towire_bitcoin_blkid(&ptr, r->chain);
	return ptr;
}
static void fromwire_tlv_invoice_request_chain(const u8 **cursor, size_t *plen, void *vrecord)
{
	struct tlv_invoice_request *r = vrecord;

	    r->chain = tal(r, struct bitcoin_blkid);

fromwire_bitcoin_blkid(cursor, plen, &*r->chain);
}
...
static u8 *towire_tlv_invoice_request_payer_key(const tal_t *ctx, const void *vrecord)
{
	...
	towire_point32(&ptr, r->payer_key);
	return ptr;
}
static void fromwire_tlv_invoice_request_payer_key(const u8 **cursor, size_t *plen, void *vrecord)
{
	struct tlv_invoice_request *r = vrecord;
	    ...
	    r->payer_key = tal(r, struct point32);

fromwire_point32(cursor, plen, &*r->payer_key);
}
...
const struct tlv_record_type tlvs_invoice_request[] = {
	...
	{ 3, towire_tlv_invoice_request_chain, fromwire_tlv_invoice_request_chain },
...
/* INVOICE MSG: chain */
static u8 *towire_tlv_invoice_chain(const tal_t *ctx, const void *vrecord)
{
	const struct tlv_invoice *r = vrecord;
	u8 *ptr;
...
static void fromwire_tlv_invoice_chain(const u8 **cursor, size_t *plen, void *vrecord)
{
	struct tlv_invoice *r = vrecord;
...
/* INVOICE MSG: issuer */
static u8 *towire_tlv_invoice_issuer(const tal_t *ctx, const void *vrecord)
{
	const struct tlv_invoice *r = vrecord;
	u8 *ptr;
...
static void fromwire_tlv_invoice_issuer(const u8 **cursor, size_t *plen, void *vrecord)
{
	struct tlv_invoice *r = vrecord;
...
static const struct tlv_record_type tlvs_invoice[] = {
	...
	{ 3, towire_tlv_invoice_chain, fromwire_tlv_invoice_chain },
	...
	{ 20, towire_tlv_invoice_issuer, fromwire_tlv_invoice_issuer },
...
{
	towire_tlv(pptr, tlvs_invoice, 28, record);
}


bool fromwire_invoice(const u8 **cursor, size_t *max, struct tlv_invoice *record)
{
	return fromwire_tlv(cursor, max, tlvs_invoice, 28, record, &record->fields);
}
...
// SHA256STAMP:9846f1d389529ec3a0971b25e53c6f3b0075301a88424dab18fae2482a821f9e
