...
const char *onchaind_wire_name(int e)
{
	...
	case WIRE_ONCHAIND_HTLCS: return "WIRE_ONCHAIND_HTLCS";
...
u8 *towire_onchaind_init_reply(const tal_t *ctx, u64 commit_num)
{
	u8 *p = tal_arr(ctx, u8, 0);
...
bool fromwire_onchaind_init_reply(const void *p, u64 *commit_num)
{
	const u8 *cursor = p;
	size_t plen = tal_count(p);
...
/* WIRE: ONCHAIND_HTLCS */
u8 *towire_onchaind_htlcs(const tal_t *ctx, const struct htlc_stub *htlc, const bool *tell_if_missing, const bool *tell_immediately)
{
	u32 num_htlcs = tal_count(tell_immediately);
	u8 *p = tal_arr(ctx, u8, 0);

	towire_u16(&p, WIRE_ONCHAIND_HTLCS);
	towire_u32(&p, num_htlcs);
	for (size_t i = 0; i < num_htlcs; i++)
		towire_htlc_stub(&p, htlc + i);
	/* If it's not in the commitment tx */
	for (size_t i = 0; i < num_htlcs; i++)
		towire_bool(&p, tell_if_missing[i]);
	for (size_t i = 0; i < num_htlcs; i++)
		towire_bool(&p, tell_immediately[i]);

	return memcheck(p, tal_count(p));
}
bool fromwire_onchaind_htlcs(const tal_t *ctx, const void *p, struct htlc_stub **htlc, bool **tell_if_missing, bool **tell_immediately)
{
	u32 num_htlcs;
	...
	if (fromwire_u16(&cursor, &plen) != WIRE_ONCHAIND_HTLCS)
		return false;
 	num_htlcs = fromwire_u32(&cursor, &plen);
 	// 2nd case htlc
	*htlc = num_htlcs ? tal_arr(ctx, struct htlc_stub, num_htlcs) : NULL;
	for (size_t i = 0; i < num_htlcs; i++)
		fromwire_htlc_stub(&cursor, &plen, *htlc + i);
 	/* If it's not in the commitment tx */
	// 2nd case tell_if_missing
	*tell_if_missing = num_htlcs ? tal_arr(ctx, bool, num_htlcs) : NULL;
	for (size_t i = 0; i < num_htlcs; i++)
		(*tell_if_missing)[i] = fromwire_bool(&cursor, &plen);
 	// 2nd case tell_immediately
	*tell_immediately = num_htlcs ? tal_arr(ctx, bool, num_htlcs) : NULL;
	for (size_t i = 0; i < num_htlcs; i++)
		(*tell_immediately)[i] = fromwire_bool(&cursor, &plen);
	return cursor != NULL;
}
...
/* onchaind->master: hey */
u8 *towire_onchaind_add_utxo(const tal_t *ctx, const struct bitcoin_outpoint *prev_out, const struct pubkey *per_commit_point, struct amount_sat value, u32 blockheight, const u8 *scriptpubkey, u32 csv_lock)
{
	u16 len = tal_count(scriptpubkey);
	u8 *p = tal_arr(ctx, u8, 0);

	towire_u16(&p, WIRE_ONCHAIND_ADD_UTXO);
	towire_bitcoin_outpoint(&p, prev_out);
	if (!per_commit_point)
		towire_bool(&p, false);
	else {
		towire_bool(&p, true);
...
bool fromwire_onchaind_add_utxo(const tal_t *ctx, const void *p, struct bitcoin_outpoint *prev_out, struct pubkey **per_commit_point, struct amount_sat *value, u32 *blockheight, u8 **scriptpubkey, u32 *csv_lock)
{
	u16 len;
	...
	if (fromwire_u16(&cursor, &plen) != WIRE_ONCHAIND_ADD_UTXO)
		return false;
 	fromwire_bitcoin_outpoint(&cursor, &plen, prev_out);
 	if (!fromwire_bool(&cursor, &plen))
...
/* transactions. */
u8 *towire_onchaind_annotate_txout(const tal_t *ctx, const struct bitcoin_outpoint *outpoint, enum wallet_tx_type type)
{
	u8 *p = tal_arr(ctx, u8, 0);

	towire_u16(&p, WIRE_ONCHAIND_ANNOTATE_TXOUT);
	towire_bitcoin_outpoint(&p, outpoint);
	towire_wallet_tx_type(&p, type);

	return memcheck(p, tal_count(p));
}
bool fromwire_onchaind_annotate_txout(const void *p, struct bitcoin_outpoint *outpoint, enum wallet_tx_type *type)
{
	const u8 *cursor = p;
	size_t plen = tal_count(p);
	...
	if (fromwire_u16(&cursor, &plen) != WIRE_ONCHAIND_ANNOTATE_TXOUT)
		return false;
 	fromwire_bitcoin_outpoint(&cursor, &plen, outpoint);
 	*type = fromwire_wallet_tx_type(&cursor, &plen);
	return cursor != NULL;
}
...
// SHA256STAMP:5cce1695099cdfa40040240ad5aa2dbb08ed8218f61cb42dccd5c16bee7d483d
