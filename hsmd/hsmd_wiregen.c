...
u8 *towire_hsmd_init_reply(const tal_t *ctx, const struct node_id *node_id, const struct ext_key *bip32, const struct point32 *bolt12, const struct secret *onion_reply_secret)
{
	u8 *p = tal_arr(ctx, u8, 0);
	...
	towire_point32(&p, bolt12);
	towire_secret(&p, onion_reply_secret);

	return memcheck(p, tal_count(p));
}
bool fromwire_hsmd_init_reply(const void *p, struct node_id *node_id, struct ext_key *bip32, struct point32 *bolt12, struct secret *onion_reply_secret)
{
	const u8 *cursor = p;
	size_t plen = tal_count(p);
	...
	if (fromwire_u16(&cursor, &plen) != WIRE_HSMD_INIT_REPLY)
 	...
 	fromwire_point32(&cursor, &plen, bolt12);
...
// SHA256STAMP:50dc3e422b6f20fea98e1492a0f234d62ea6cdd2428177485db74178b056d617
